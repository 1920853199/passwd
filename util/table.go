...
func Output(slice interface{}) {
	fmt.Println(Table(slice, false))
}

func OutputLine(slice interface{}) {
	fmt.Println(Table(slice, true))
}
...
func Table(slice interface{}, l bool) string {
	coln, colw, rows, err := parse(slice)
	if err != nil {
		return err.Error()
	}
	table := table(coln, colw, rows, m["box-drawing"], l)
	return table
}
...
func AsciiTable(slice interface{}) string {
	...
	table := table(coln, colw, rows, m["ascii"], false)
	return table
}
...
func table(coln []string, colw []int, rows [][]string, b bd, l bool) (table string) {
	if len(rows) == 0 {
		return ""
	}
	...
	for i, v := range colw {
		...
		head[1] = append(head[1], []rune(" "+"\x1b[32m\x1b[1m"+coln[i]+repeat(v-StringLength([]rune(coln[i]))+1, ' ')+"\x1b[0m"+string(b.V))...)
		head[2] = append(head[2], []rune(repeat(v+2, b.H)+string(b.VH))...)
		line = append(line, []rune(repeat(v+2, b.H)+string(b.VH))...)
		bttm = append(bttm, []rune(repeat(v+2, b.H)+string(b.HU))...)
	}
	head[0][len(head[0])-1] = b.DL
	head[2][len(head[2])-1] = b.VL
	line[len(line)-1] = b.VL
